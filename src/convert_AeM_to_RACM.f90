...
subroutine convert_AeM_to_racm(isp,iespc,ident,source_type,spc_name)
!use chem1_list
...
if(spc_name == 'NO') then
   ...
   if(use_bbem   == 1 .or. use_gfedv2 == 1) found_emiss_spc(iespc,bburn)    = 1
...
if(spc_name == 'CSL') then
   ...
   iespc=iespc+1
   emiss_spc_name(iespc,bburn)           = spc_name
   if(use_bbem   == 1) emiss_g(iespc)%src_bburn(:,:,1)   = 108/94* bbbem_g(ident)%src(:,:,1)
...
if(spc_name == 'TOL') then
   ...
   if(use_bbem   == 1) emiss_g(iespc)%src_bburn(:,:,1)   = 92/82*bbbem_g(benzene	)%src(:,:,1) + &
...
if(spc_name == 'HC8') then
   ...
   iespc=iespc+1
   emiss_spc_name(iespc,bburn)           = spc_name
   if(use_bbem	== 1) emiss_g(iespc)%src_bburn(:,:,1) = 114/100*bbbem_g(heptane         )%src(:,:,1) + &
...
if(spc_name == 'OLI') then
   ...
   iespc=iespc+1
   emiss_spc_name(iespc,bburn)           = spc_name
   if(use_bbem	== 1) emiss_g(iespc)%src_bburn(:,:,1) = 68/56*bbbem_g(butene_tr_2 )%src(:,:,1) + &
...
if(spc_name == 'HC3') then
   ...
   iespc=iespc+1
   emiss_spc_name(iespc,bburn)           = spc_name
   if(use_bbem	== 1) emiss_g(iespc)%src_bburn(:,:,1) = 44/72*bbbem_g(butane_2_Me)%src(:,:,1) + &
  							bbbem_g(C3H8       )%src(:,:,1) + &
...
if(spc_name == 'HC5') then
   ...
   iespc=iespc+1
   emiss_spc_name(iespc,bburn)           = spc_name
   if(use_bbem	== 1) emiss_g(iespc)%src_bburn(:,:,1) = bbbem_g(pentane_n )%src(:,:,1) + &
...
end subroutine convert_AeM_to_racm
