...
!-----------------------------------------------------------------------------------------------------------------------!
       ...
       8.92 ,	7.98,   7.04,  6.81,   8.92, 27.93,  20.19, 12.44,  0.00,  0.00, &!2   M1 ETNA HIJACK				!
...
! volume_m3            = volume_km3 * 1000**3     
  integer, parameter :: maxnspecies = 200 , nspecies = 20 
  integer, parameter ::         &
   ASH      = 1 & 
  ,SO2      = 2 &
  ,H2O      = 3 &
...
!   
  ...
  use grid_dims_out, only : volcano_index,use_these_values&
                            ,vso2_er  ,vh2o_er  ,vsulf_so2 &
                            ,vbr_so2  ,vhbr_so2 ,vhobr_so2,vbr2_so2 ,vbro_so2 &
                            ,vcl_so2  ,vhcl_so2 ,vhocl_so2,vcl2_so2 ,vclo_so2 &
                            ,vho_so2  ,vho2_so2 &
                            ,vno_so2  ,vno2_so2 ,vhno3_so2 &
                            ,vhg_so2 &
                            ,vash_er,vash_dist,vemis_elev,vbase_elev
  use volcanoes_emissions, only :volcanoes_name,	volc_prop,nvolcanoes,nspecies,&
  	 LATI,LONG,ELEV,BEGT,CRAT,HEAT,INJH,DURA,MASH,&
  	 MSO2,MH2O,MSULF,&
  	 MBR,MHBR,MHOBR,MBR2,MBRO,&
  	 MCL,MHCL,MHOCL,MCL2,MCLO,&
  	 MHO,MHO2,&
  	 MNO,MNO2,MHNO3,&
  	 MHG
  implicit none
  ...
  print*,"DEBUG1 I think vhobr_so2 is", vhobr_so2
  ...
  !-- model grid box, if is it the case.
    ...
    call get_emission_per_volcano(volcano_index,use_these_values&
		 ...
		 , volc_prop(MH2O)%volcano  &
         , volc_prop(MSULF)%volcano &
         , volc_prop(MBR)%volcano   &
         , volc_prop(MHBR)%volcano  &
         , volc_prop(MHOBR)%volcano &
         , volc_prop(MBR2)%volcano  &
         , volc_prop(MBRO)%volcano  &
         , volc_prop(MCL)%volcano   &
         , volc_prop(MHCL)%volcano  &
         , volc_prop(MHOCL)%volcano &
         , volc_prop(MCL2)%volcano  &
         , volc_prop(MCLO)%volcano  &
         , volc_prop(MHO)%volcano   &
         , volc_prop(MHO2)%volcano  &
         , volc_prop(MNO)%volcano   &
         , volc_prop(MNO2)%volcano  & 
         , volc_prop(MHNO3)%volcano & 
         , volc_prop(MHG)%volcano   &
                    ,vemis_elev,vbase_elev &
                    ,vso2_er  ,vh2o_er  ,vsulf_so2 &
                    ,vbr_so2  ,vhbr_so2 ,vhobr_so2,vbr2_so2 ,vbro_so2 &
                    ,vcl_so2  ,vhcl_so2 ,vhocl_so2,vcl2_so2 ,vclo_so2 &
                    ,vho_so2  ,vho2_so2 &
  ...
  print*,"DEBUG2 I think vhobr_so2 is", vhobr_so2
  !print*,"DEBUG2 I think max(volc_prop(MSO2) is", max(volc_prop(MSO2)%volcano)
  !print*,"DEBUG2 I think max(volc_prop(MHBR) is", max(volc_prop(MHBR)%volcano)
  !print*,"DEBUG2 I think max(volc_prop(MBR2) is", max(volc_prop(MBR2)%volcano)
...
!print*,"DEBUG3 I think max(volc_prop(MSO2) is", max(volc_prop(MSO2)%volcano)
!print*,"DEBUG3 I think max(volc_prop(MHBR) is", max(volc_prop(MHBR)%volcano)
!print*,"DEBUG3 I think max(volc_prop(MBR2) is", max(volc_prop(MBR2)%volcano)
...
subroutine get_emission_per_volcano(volcano_index,use_these_values&
                           ...
                           ,QMASH &
                           ,QMSO2,QMH2O,QMSULF &
                           ,QMBR,QMHBR,QMHOBR,QMBR2,QMBRO &
                           ,QMCL,QMHCL,QMHOCL,QMCL2,QMCLO &
                           ,QMHO,QMHO2 &
                           ,QMNO,QMNO2,QMHNO3 &
                           ,QMHG &
                           ,vemis_elev,vbase_elev &
...
real, intent(in) :: vemis_elev, vbase_elev &
        ,vso2_er  ,vh2o_er  ,vsulf_so2 &
        ,vbr_so2  ,vhbr_so2 ,vhobr_so2,vbr2_so2 ,vbro_so2 &
        ,vcl_so2  ,vhcl_so2 ,vhocl_so2,vcl2_so2 ,vclo_so2 &
        ,vho_so2  ,vho2_so2 &
...
real, dimension(10), intent(in) :: vash_dist

integer i1,i2,iesp,ifoc,ashbin
...
do ifoc = i1,i2
      ...
      QELEV(ifoc)=volcanoes_data(Elevation,ifoc) +vbase_elev
      QINJH(ifoc)=vemis_elev !meters
...
!- only works for 1 volcano
      do ashbin = 1,10
         ash_size_dist(ashbin)=vash_dist(ashbin)/sum(vash_dist)
      end do
!      print*,'ash size dist=', int(volcanoes_data(esp,ifoc)),ash_size_dist(:)*100.

      QMSO2(ifoc)= vso2_er * QDURA(ifoc)   ! kg for now
      QMH2O(ifoc)= vh2o_er * QDURA(ifoc)   ! kg for now
! Ratios are defined by user
      !S spcies
      QMSULF(ifoc)=QMSO2(ifoc) * vsulf_so2
      !Br species
      QMBR(ifoc)   = QMSO2(ifoc) * vbr_so2
      QMHBR(ifoc)  = QMSO2(ifoc) * vhbr_so2
      QMHOBR(ifoc) = QMSO2(ifoc) * vhobr_so2
      QMBR2(ifoc)  = QMSO2(ifoc) * vbr2_so2
      QMBRO(ifoc)  = QMSO2(ifoc) * vbro_so2
      !Cl species
      QMCL(ifoc)   = QMSO2(ifoc) * vcl_so2
      QMHCL(ifoc)  = QMSO2(ifoc) * vhcl_so2
      QMHOCL(ifoc) = QMSO2(ifoc) * vhocl_so2
      QMCL2(ifoc)  = QMSO2(ifoc) * vcl2_so2
      QMCLO(ifoc)  = QMSO2(ifoc) * vclo_so2
      !HOx species
      QMHO(ifoc)   = QMSO2(ifoc) * vho_so2
      QMHO2(ifoc)  = QMSO2(ifoc) * vho2_so2
      !NOy species
      QMNO(ifoc)   = QMSO2(ifoc) * vno_so2
      QMNO2(ifoc)  = QMSO2(ifoc) * vno2_so2
      QMHNO3(ifoc) = QMSO2(ifoc) * vhno3_so2
      !Hg species 
      QMHG(ifoc)   = QMSO2(ifoc) * vhg_so2 
...
!print*, ifoc ,QLONG(ifoc), QLATI(ifoc), QELEV(ifoc), QINJH(ifoc), QDURA(ifoc), QMASH(ifoc)
      ...
      do iesp=1,nspecies
       ...
       if(iesp==h2o)qsc(ifoc,iesp) = QMH2O(ifoc)
       if(iesp==sulf)qsc(ifoc,iesp)= QMSULF(ifoc)
       !Br species
       if(iesp==br)qsc(ifoc,iesp)  = QMBR(ifoc)
       if(iesp==hbr)qsc(ifoc,iesp) = QMHBR(ifoc)
       if(iesp==hobr)qsc(ifoc,iesp)= QMHOBR(ifoc)
       if(iesp==br2)qsc(ifoc,iesp) = QMBR2(ifoc)
       if(iesp==bro)qsc(ifoc,iesp) = QMBRO(ifoc)
       !Cl species
       if(iesp==cl)qsc(ifoc,iesp)  = QMCL(ifoc)
       if(iesp==hcl)qsc(ifoc,iesp) = QMHCL(ifoc)
       if(iesp==hocl)qsc(ifoc,iesp)= QMHOCL(ifoc)
       if(iesp==cl2)qsc(ifoc,iesp) = QMCL2(ifoc)
       if(iesp==clo)qsc(ifoc,iesp) = QMCLO(ifoc)
       !HOx species
       if(iesp==ho)qsc(ifoc,iesp)  = QMHO(ifoc)
       if(iesp==ho2)qsc(ifoc,iesp) = QMHO2(ifoc)
       !NOx species
       if(iesp==no)qsc(ifoc,iesp)  = QMNO(ifoc)
       if(iesp==no2)qsc(ifoc,iesp) = QMNO2(ifoc)
       if(iesp==hno3)qsc(ifoc,iesp) = QMHNO3(ifoc)
       !hg species
       if(iesp==hg)qsc(ifoc,iesp)  = QMHG(ifoc)
...
!print*, 'searching for the following volcanic species:'
!print*, volcanoes_spc_name
...
!  if(spc_name == volcanoes_spc_name(isp) .or. spc_name =='V_'//trim(volcanoes_spc_name(isp))//'1') then
  ...
  if(spc_name == volcanoes_spc_name(isp)) then 
      ...
      return
  !else
  !    print*,'==>volcanoes NOT found for ',spc_name,isp
   endif
enddo
