...
!     IF SET TO ZERO IN NAMELIST
   ...
   do ifm = 1,ngrids
      ...
      if (icm >= 1) then
         ...
         ! - 1. * deltayn(ifm)
         print*, 'ifm =',ifm
         print*, 'xmn(1,ifm) =',xmn(1,ifm)
         print*, 'ymn(1,ifm) =',ymn(1,ifm); call flush(6)
      endif
   enddo
endif
...
do ifm = ngra,ngrb
   icm = nxtnest(ifm)
   if (icm >= 1) then
      xmn(1,ifm) = xmn(ninest(ifm),icm) - deltaxn(ifm)!LSURL
      ymn(1,ifm) = ymn(njnest(ifm),icm) - deltayn(ifm)!LSURL
      print*, 'Q - ifm =',ifm
      print*, 'Q - xmn(1,ifm) =',xmn(1,ifm)
      print*, 'Q - ymn(1,ifm) =',ymn(1,ifm); call flush(6)
   endif
   do i = 2,nnxp(ifm)
